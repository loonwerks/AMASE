...
public class TransformAgree implements AgreeAutomater {
	...
	public AgreeProgram transform(AgreeProgram program) {
		...
		try{
			...
			SystemImplementationImpl sysimpl = (SystemImplementationImpl) (program.topNode.reference);
			SafetyContractSubclause safetyannex = getSafetyAnnex(sysimpl);

			// extra check to make sure we are doing something sensible.
			if (TransformAgree.transformFlag && safetyannex != null) {
				System.out.println(
						"Perform Safety Analysis is true and annex exists, transforming original agree program");
			...
			System.out.println("Error occurred performing safety analysis transformation");
		}
		// And return program
		return program;
	}
