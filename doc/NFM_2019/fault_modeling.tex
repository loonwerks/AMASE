...
\janet{Also consider revising sub-section titles to be consistent with the style used in other sections, e.g., capitalize each main word.}
...
\danielle{D's comment: when the previous paragraph was written, I checked through the ARP definitions and described them accordingly. I will try and catch all of my mistakes in the following sections. We will probably have to proof read this section a few times. It is slightly confusing using these three terms correctly and at the same time using the generic term ``fault'' as in the safety annex node definition.}
