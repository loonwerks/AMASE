...
Inadvertent braking means that braking force is applied at the wheel but the pilot has not pressed the brake pedal.  In addition, the definition requires that power and hydraulic pressure are both present, the plane is not stopped, and the wheel is rolling (not skidding). \danielle{This AGREE contract is shown in Figure~\ref{fig:inadvertent_braking}.}
\begin{figure}[h!]
	\vspace{-0.2in}
	\begin{center}
