...
%There are a total of 33 different fault types and 141 fault instances within the model.  The large number of fault instances is due to the redundancy in the system design and its replication to control 8 wheels. 

%An example top-level property is to ensure that there is no inadvertent braking of the wheels.  This means that if power and hydraulic pressure is supplied (i.e., braking is commanded), then one of the following must be true: the mechanical pedal is pressed, brake force is zero, or the wheel is not rolling.
