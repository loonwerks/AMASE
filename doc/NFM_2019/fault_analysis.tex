...
\janet{TODO: consider combine the first three subsections and update the last section to talk from users/use case stand point of view - walk through concrete examples to show how we use the analysis capabilities mentioned in the original subsections to help address the problem we raised in the intro/prelim section}
...
\janet{TODO: replace timing data on something about what we learned from the analysis results, and use it to feedback to the system design, and rerun the analysis. As Mats pointed out, "can we add a redundant sensor and get fault tolerance?}
