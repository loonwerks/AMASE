...
\danielle{This algorithm should be different it seems to me. There are some instances where adding the destination to the source set after the initial algorithm terminates might leave out faults that should be considered. The destination should be automatically added to the source set at the time of computation.}

\subsection{Compositional Analysis}
In compositional analysis, the analysis proceeds in a top down fashion. To prove the top level properties, the %contracts
\janet{properties} in the layer directly beneath the top level are used to perform the proof. The analysis proceeds in this manner. %Currently it is possible to assign a maximum number of faults to a layer of the architecture. 

%When this number of faults is constrained, it occurs during the analysis of that layer.
 \janet{Users can constrain the maximum number of faults within each layer of the model by specifying the maximum fault hypothesis statement to that layer. } 
 %This means that it is clear which of the guarantees of that layer are violated by the active faults, but we have not yet added the ability to propagate this information up to higher layers in the architecture. 
 \janet{ If any lower level property failed due to activation of faults, the property verifications at the higher level can no longer be trusted as the higher level properties were proved based on the facts that its direct sub level properties are valid.}
 
 This type of analysis is helpful to see weaknesses in a given component or layer of the system. %, but the reflection of these active faults is not displayed system wide.
\janet{We plan to reflect lower level property violations in the verification results of higher layers in the architecture, and enable displaying or constraining active faults system wide instead of component wide.}
