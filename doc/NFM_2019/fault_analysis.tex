...
In this section, we describe the architecture of the Safety Annex that supports the analysis, and describe the two possible fault analyses available to users, monolithic and compositional. 
...
For fault analysis, we separate the possible analyses available to users into two distinct actions , monolithic and compositional, and describe them here.
\end{comment}
...
When monolithic analysis is performed on the nominal system model, the architectural model is flattened in order to perform the analysis. All of the contracts in the lower levels are used for the analysis. %In order to perform probabilistic fault analysis, monolithic analysis is required. 
...
\janet{should we keep the following two paragraphs on the timing?}
...
%As indicated in Figure~\ref{fig:interaction_with_FTA}, the sync and update between the preliminary system FTA and
