...
In the development of complex airborne systems, safety assessment is a starting point at each hierarchical level of the development life cycle. Safety analysts need to understand the system content and behavior, provide safety analysis results, and compare them with safety objectives and requirements to show compliance. Acquiring the required knowledge about how the software functions are intended to work in such systems has shown in practice a very involved and time consuming task. Existing approaches that annotate the system architecture model with failure modes and fault propagations help safety analysts to better communicate with system designers and address the complexity of the system. However, knowledge on how the faults propagate through the components still needs to be acquired prior to such information can be captured in the model. The methodology described in this paper allows safety analysts to specify faults and faulty behaviors at individual components level (using the Safety Annex for the Architecture Analysis and Design Language (AADL)). The provided tool support weaves the faults into the nominal system model provided by system designers. No explicit fault propagation is necessary since the faulty behavior propagates through the system as normal behavior in the nominal system model. The behavior of the system in the presence of faults are verified using model checking through Assume Guarantee Reasoning Environment (AGREE).
The effects of any triggered fault are manifested in the formal analysis results.
