...
The fault library contains commonly used fault node definitions. An example of a fault node is shown below: \janet{rename ``non-deterministic fault'' to ``pressure-output fail to close fault'' in the text and in the figure so don't associate ``non-deterministic'' with fault? Also, the way we define the faulty behaviors, sounds like they are failure modes/failures?}
\begin{figure}[h!]
	\vspace{-0.19in}
	\begin{center}
...
\begin{figure}[h!]
	...
	\begin{center}
		...
		\includegraphics[trim=0 9 0 5,clip,width=1.0\textwidth]{images/pump_fault.png}
	\end{center}
	\vspace{-0.40in}
\end{figure}
...
\subsection{Hardware Failures and Dependent Fault}
