...
\janet{We examine scalability by looking at the analysis time. For the nominal system model (no faults), it took monolithic analysis (where the entire system is analyzed in one pass) 26 seconds to analyze, and 2 minutes and 5 seconds for compositional analysis (where the verification task is split into layers to be analyzed separately). The overhead in compositional analysis was due to the add-up startup cost to invoke the model checker for individual systems. For the model with faulty behavior when given a single-fault hypothesis, compositional analysis took 2 minutes 6 seconds, and monolithic analysis got an out-of-memory error after 29 minutes with a JVM memory limit of 12GB. This clearly demonstrates the value of compositional analysis for more complex models. For the model with faulty behavior with probabilistic hypothesis, the monolithic analysis took 4 minutes, and compositional analysis took 2 minutes 6 seconds. However, we have to rely on the analysis results from monolithic analysis for faulty behavior with probabilistic hypothesis, as the tool support is not yet in place for aggregated probabilities in compositional setting.}
